...
public class SamsungExynos4RIL extends RIL implements CommandsInterface {
    static final String LOG_TAG = "SamsungExynos4RIL";
    //SAMSUNG STATES
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
    ...
    */
    ...
    responseCallList(Parcel p) {
        int size;
        DriverCall dc;
        ArrayList<DriverCall> response = (ArrayList<DriverCall>)super.responseCallList(p);
	size = response.size();
        for (int i = 0; i < size; i++) {
	    dc = (DriverCall) response.get(i);
	    dc.index &= 0xFF;
	}
        return response;
    }
